              ...
              <div className='sm:scale-75 md:scale-75 lg:scale-100 xl:scale-105 2xl:scale-110 sm:max-2xl:flex justify-start'>
                  <form action="post" onSubmit={handleSubmit} className='sm:p-5 md:p-9 lg:p-10 xl:p-11 2xl:p-11  ' >
                    ...
                    <div className="flex justify-between">
                      ...
                      {error ? <h3 className='text-xs text-red-700'>Valid email required</h3> : <></>}
                    </div>
                      <input type="email" placeholder='email@company.com' id="email" onChange={(e) => setEmail(e.target.value)}
                      onInvalid={(e) => {
                        e.preventDefault();
                        setError(true);
                      }} 
                      className='text-xs p-3 my-1 border-2 rounded-lg outline-0 focus:outline-2 outline-slate-500 ' />
                    </div>
                    <button className='text-xs text-white bg-slate-800 p-3 rounded-lg w-full mt-2 hover:bg-gradient-to-r  hover:from-tomato hover:to-orange-400 focus:bg-gradient-to-r  focus:from-tomato focus:to-orange-400 hover:shadow-2xl shadow-current'>Subscribe to monthly newsletter</button>
                  </form>
                  <img src={image} alt="" />
                </div>  
              </div>
